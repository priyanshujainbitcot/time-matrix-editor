            ...
            <div className={`fixed top-0 right-0 h-full w-80 bg-white border-l border-gray-200 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-gray-50/80">
                    ...
                    <div className="flex items-center gap-1">
                        {items.length > 1 && (
                            <button onClick={onClearAll} className="px-2 py-1 text-[11px] text-gray-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors cursor-pointer" title="Dismiss all">Clear all</button>
                        )}
