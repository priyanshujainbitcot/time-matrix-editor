...
export default function ShowNotesPage() {
    ...
    const handleExportNote = async () => {
        ...
        try {
            ...
            // Standard font mapping using only Roboto
            pdfMake.fonts = { Roboto: robotoMapping };
            ...
            // 1. Wipe out font families from HTML before processing
            const allElements = doc.querySelectorAll('*');
            for (let i = 0; i < allElements.length; i++) {
                (allElements[i] as HTMLElement).style.fontFamily = '';
            }
            ...
            // 2. ULTIMATE KILL-SWITCH: Recursively strip any rogue font properties 
            // generated by html-to-pdfmake (like from <code> tags making monospace fonts)
            const stripFonts = (node: any) => {
                if (Array.isArray(node)) {
                    node.forEach(stripFonts);
                } else if (node !== null && typeof node === 'object') {
                    if (node.font) delete node.font; // Force pdfmake to fallback to defaultStyle
                    Object.values(node).forEach(stripFonts);
                }
            };
            stripFonts(translatedContent);
